def main() -> int:
    ...
    circuits = build_circuits(sorted_distances, 1000)
    top_three = circuits[:3]
    top_three_lens = [len(c) for c in circuits[:3]]
    result = 1

    for l in top_three_lens:
        result *= l
...
def build_circuits(distances: list[tuple[int, tuple, tuple]], n: int) -> list[tuple]:
    point_to_circuit = {}
    counter = 0
    ...
    circuits = set(tuple(v) for v in point_to_circuit.values())
